...
class DAT(nn.Module):
    ...
    def forward(self, x):
        ...
        x1, pos, ref = self.stages[0](x)
        x = self.down_projs[0](x1)
        positions.append(pos)
        references.append(ref)

        x2, pos, ref = self.stages[1](x)
        x = self.down_projs[1](x2)
        positions.append(pos)
        references.append(ref)

        x3, pos, ref = self.stages[2](x)
        x = self.down_projs[2](x3)
        positions.append(pos)
        references.append(ref)

        x4, pos, ref = self.stages[3](x)
        positions.append(pos)
        references.append(ref)
